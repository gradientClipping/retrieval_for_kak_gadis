...
ADAPTER_PATH = "/Users/juan.wijaya/Documents/AI Thingy/Knowledge Management/KM Assistant/Retrieval/qwen3-lora-pas-ir-optimized2"
BASE_MODEL_ID = "Qwen/Qwen3-Embedding-4B" # Replace with the specific Qwen version used for the adapter
# API_KEY = st.secrets.get("OPENROUTER_API_KEY", "Bearer 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") # Better to use secrets
...
def load_local_model():
    """Loads the tokenizer and model with LoRA adapter locally."""
    ...
    try:
        ...
        try:
            ...
            st.error(f"Could not load adapter from {ADAPTER_PATH}. Error: {str(e)}")
            st.warning("Running without Adapter (Base model only) or check path.")
...
class SearchEngine:
    def __init__(self, alpha=0.4, beta=0.4, gamma=0.2, use_bm25=True, bm25_weight=0.3, temperature=1.0):
        ...
        # Load model once
        self.tokenizer, self.model, self.device = load_local_model()

    def embed(self, text: str) -> np.ndarray:
        if self.model:
            return get_embedding_local(text, self.tokenizer, self.model, self.device)
        else:
            # Fallback to random for testing if model fails to load
    ...
    def search(self, query_init, query_corrected, top_k=5, retrieval_k=50):
        # 1. Embed Corrected Query
        q_corrected_emb = self.embed(query_corrected)
        
        # 2. Initial Retrieval (Hybrid: Dense + BM25)
...
if 'search_engine' not in st.session_state:
    with st.spinner("Initializing Search Engine & Indexing KB..."):
